...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	...
	case station.Table:
		query := c.Station.Query().
			Where(station.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, stationImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	case station.Table:
		...
		query, err := query.CollectFields(ctx, stationImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
